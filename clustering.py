...
class Clustering:
    ...
    def hdbscan_clustering(
        self, documents: list[dict], min_cluster_size: int = 5, min_samples: int = 5
    ) -> list[list[dict]]:
        if len(documents) == 0:
            return [[]]
    ...
    def kmeans_clustering(self, documents: list[dict], n_clusters: int = 5) -> list[list[dict]]:
        ...
        # plotting the clusters in clusters.png
        # plt.scatter(embeddings_2d[:, 0], embeddings_2d[:, 1], c=labels, cmap='viridis')
        # plt.title('K-means Clustering of Documents (t-SNE Reduced)')
    ...
    def smooth_sample_clusters(
        self, clusters: list[list[dict]], laplace_smoothing: float, top_k: int
    ) -> list[dict]:
        sizes = [len(c) for c in clusters]
        sizes_dist = [n / sum(sizes) for n in sizes]
        smoothed_sizes_dist = [
            (s + laplace_smoothing) / (1 + len(sizes) * laplace_smoothing) for s in sizes_dist
        ]
